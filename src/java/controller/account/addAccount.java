...
response.setCharacterEncoding("UTF-8");
        try (PrintWriter out = response.getWriter()) {
            ...
            AccountDTO accountDTO = new AccountDTO(account, pass, lastName, firstName, birthday, gender, phone, isUse, role);
            AccountDAO accountDAO = new AccountDAO();
            
             request.getSession().removeAttribute("mess error");
            int result=0;
            try {
                result = accountDAO.add(accountDTO);
            } catch (Exception e) {
                out.print(e);
            }
            if (result > 0) {
                request.getRequestDispatcher("private/listOfAccount.jsp").forward(request, response);
            } else {
                request.getSession().setAttribute("mess error", "account exist !!");
